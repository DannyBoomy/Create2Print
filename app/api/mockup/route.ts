...
export async function POST(req: NextRequest) {
  try {
    ...
    // Step 2: Create a temporary product to get mockup
    ...
    const mockupUrl = mockupImages[0]?.src || null
    ...
    return NextResponse.json({ mockupUrl, printifyImageId })

  } catch (error: any) {
    console.error('Mockup error:', error?.response?.data || error)
    // Return null mockup gracefully — app still works, just shows flat image
    return NextResponse.json({ mockupUrl: null, printifyImageId: null })
  }
}
