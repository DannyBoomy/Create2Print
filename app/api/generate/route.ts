...
export async function POST(req: NextRequest) {
  try {
    ...
    // Fallback to gpt-image-2 if 2.5 not available on account yet
    if (error?.message?.includes('model') || error?.status === 404) {
      try {
        const { prompt, width, height } = await req.json().catch(() => ({}))
        const size = getOpenAIImageSize(width || 18, height || 24)
        ...
        const imageUrl = fallback.data?.[0]?.url
        const b64 = fallback.data?.[0]?.b64_json
        if (imageUrl || b64) {
          return NextResponse.json({ imageUrl: imageUrl || `data:image/png;base64,${b64}`, generationsLeft: 0, size })
        }
      } catch {}
    }
    return NextResponse.json(
